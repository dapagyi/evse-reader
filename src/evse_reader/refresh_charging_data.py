...
def login(base_url: str, username: str, password: str) -> requests.Session:
    ...
    data = {"lang": "en", "login": username, "password": password, "remember": "on"}
...
def download_charging_data_csv(session: requests.Session, base_url: str) -> None:
    ...
    initial_url = (
        f"{base_url}/cgi-bin/cgiServer?worker=DisplayChargeDetails"
        "&cdrType=manual&cdrFileName=/tmp/export.csv&longProcessing=true"
    )
...
def refresh_charging_data(db: sqlite3.Connection, base_url: str, username: str, password: str) -> None:
